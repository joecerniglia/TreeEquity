...
'Washington'='WA','West Virginia'='WV','Wisconsin'='WI','Wyoming'='WY')), 
            ...
            checkboxInput("colorblind", "color-blind", FALSE),
...
tags$footer(
  ...
  tags$a(
   ...
   " Template for statistical source code: ",
  tags$a(
...
server <- function(input, output) {
  ...
  output$geo <- renderPlot({
    ...
    if (input$colorblind==TRUE) {Gradi<-"#0000FF"} else {Gradi<-"#013220"}
    ...
    p2 <- if(p_value<=.001){"[global p < 0.001]"}
    else {paste0("[global p=",round(p_value,3),"]")}
    #print(p2)
